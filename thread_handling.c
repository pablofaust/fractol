...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/20 18:49:23 by pfaust            #+#    #+#             */
/*   Updated: 2018/03/20 18:49:23 by pfaust           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_screen	**init_args(t_screen **screens, int nbr_scr, t_env *env)
{
	...
	init_arg_limits(env->screen[nbr_scr]->min_x +
			env->screen[nbr_scr]->width / 2,
			env->screen[nbr_scr]->min_y, screens[1]);
	init_arg_limits(env->screen[nbr_scr]->min_x,
			env->screen[nbr_scr]->min_y + env->screen[nbr_scr]->height / 2,
			screens[2]);
	init_arg_limits(env->screen[nbr_scr]->min_x +
			env->screen[nbr_scr]->width / 2, env->screen[nbr_scr]->min_y +
			env->screen[nbr_scr]->height / 2, screens[3]);
	return (screens);
}
